...
class IMUXSocket
	...
	def recieve_connection(server)
		# instead, session object opens server and created workers by passing in sockets
	...
	def serve_download(file_queue)
		...
		loop{
			...
			chunk = nil
			@server_semaphore.synchronize{ chunk = file_queue.get_next_chunk }	# where should the queue come from?
			if chunk == nil
				@socket.puts "DONE"
